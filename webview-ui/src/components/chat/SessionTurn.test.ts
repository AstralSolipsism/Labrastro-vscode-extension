...
describe("SessionTurn source order", () => {
  ...
  it("renders running process and reasoning presentation items by stable keys", () => {
    ...
    expect(source).toContain("processTimelineItemKey")
    ...
    expect(source).toContain("<KeyedFor each={props.items} key={processTimelineItemKey}>")
    expect(source).not.toContain("<Index each={presentation()}>")
    expect(source).not.toContain("<Index each={props.items}>")
  ...
  it("uses the Rose Four loader for running reasoning and process groups only", () => {
    ...
    const processSummaryStart = source.indexOf("const ProcessSummaryPart")
    const reasoningPanelSource = source.slice(reasoningPanelStart, timelineProcessGroupStart)
    const processGroupSource = source.slice(timelineProcessGroupStart, processSummaryStart)
