...
describe("settings architecture", () => {
  ...
  it("uses controller-owned first-load states for agent config and capabilities", () => {
    const source = readFileSync(controllerPath, "utf8")
    ...
    expect(source).toContain("const pageInitialLoading = (tab: SettingsTab): boolean =>")
    expect(source).toContain("const pageRevalidating = (tab: SettingsTab): boolean =>")
    expect(source).toContain("const pageLoadingMessage = (tab: SettingsTab): string =>")
    expect(agentConfigSource).toContain('pageInitialLoading("agentConfig")')
    expect(agentConfigSource).toContain('pageRevalidating("agentConfig")')
  ...
  it("keeps capabilities first paint independent from dependency manifest loading", () => {
    ...
    const refreshCapabilitiesIndex = source.indexOf("const refreshCapabilities = () =>")
    const refreshCapabilitiesEnd = source.indexOf("\n  const refreshCapabilityDependencies", refreshCapabilitiesIndex)
    expect(refreshCapabilitiesIndex).toBeGreaterThanOrEqual(0)
    expect(refreshCapabilitiesEnd).toBeGreaterThan(refreshCapabilitiesIndex)
    expect(source.slice(refreshCapabilitiesIndex, refreshCapabilitiesEnd)).toContain('refreshOperation("capabilities")')
  ...
  it("keeps memory settings provider-centric and avoids unfinished tool controls", () => {
    const sessionPolicySource = readFileSync(join(tabsDir, "SessionPolicyTab.tsx"), "utf8")
    const memorySource = readFileSync(join(tabsDir, "MemoryTab.tsx"), "utf8")
    const agentConfigSource = readFileSync(join(tabsDir, "AgentConfigTab.tsx"), "utf8")
    const memoryPresentationSource = readFileSync(join(__dirname, "memoryPresentation.ts"), "utf8")

    expect(memorySource).toContain("memoryDefaultProvider")
    expect(memorySource).toContain("memoryProvidersJson")
    expect(memorySource).toContain("memorySourcesJson")
    expect(memorySource).toContain("memoryStatus")
    expect(memorySource).toContain('t("memorySettings.providerSection.title")')
    expect(memorySource).toContain('t("memorySettings.agentSection.title")')
    expect(memorySource).toContain('t("memorySettings.sourceSection.title")')
    expect(memorySource).toContain('t("memorySettings.sourceSection.desc")')
    expect(memorySource).toContain('t("memorySettings.advanced.title")')
    expect(memorySource).toContain("memoryStatusReasonKey")
    expect(memorySource).toContain("memoryStatusActionKey")
    expect(memorySource).toContain('from "../memoryPresentation"')
    expect(memorySource).toContain("memoryProviderStatuses")
    expect(memorySource).toContain("memoryAgentPolicies")
    expect(memorySource).toContain("memorySourceStatuses")
    expect(memorySource).toContain("memoryToolsEnabled")
    expect(memorySource).toContain("memoryToolsPolicy")
    expect(memorySource).toContain("default_provider")
    expect(memorySource).toContain("providers")
    expect(memorySource).toContain("sources")
    expect(memorySource).toContain("tools")
    expect(agentConfigSource).toContain("memoryProviderOptions")
    expect(agentConfigSource).toContain("memoryProvider")
    expect(agentConfigSource).toContain('t("agentConfig.agent.memoryProvider")')
    expect(agentConfigSource).toContain("selectedMemoryProviderNeedsAttention")
    expect(agentConfigSource).toContain('t("agentConfig.agent.memoryProvider.warning"')
    expect(agentConfigSource).toContain('from "../memoryPresentation"')
    expect(agentConfigSource).not.toContain("function memoryProviderStatusLabel")
    expect(memorySource).not.toContain("export function memoryStatusTone")
    expect(memorySource).not.toContain("export function memoryStatusLabel")
    expect(memorySource).not.toContain("export function memoryStatusReason")
    expect(memorySource).not.toContain("export function memoryStatusAction")
    expect(memorySource).not.toContain("function sourceSectionTone")
    expect(memorySource).not.toContain("function agentSectionTone")
    expect(memoryPresentationSource).toContain("export function memoryStatusTone")
    expect(memoryPresentationSource).toContain("export function memoryStatusLabelKey")
    expect(memoryPresentationSource).toContain("export function memoryStatusReasonKey")
    expect(memoryPresentationSource).toContain("export function memoryStatusActionKey")
    expect(memoryPresentationSource).toContain("export function memoryProviderOptionLabel")
    expect(memoryPresentationSource).toContain("export function agentSectionTone")
    expect(memoryPresentationSource).not.toMatch(/[\u4e00-\u9fff]/)
    expect(sessionPolicySource).toContain("tool_output")
    expect(sessionPolicySource).toContain("context")
    expect(sessionPolicySource).not.toContain("memoryDefaultProvider")
    expect(sessionPolicySource).not.toContain("memoryProvidersJson")
    expect(sessionPolicySource).not.toContain("memorySourcesJson")
    expect(sessionPolicySource).not.toContain("memoryStatus")
    expect(sessionPolicySource).not.toContain("memorySettings.providerSection")
    expect(sessionPolicySource).not.toContain("memorySettings.agentSection")
    expect(sessionPolicySource).not.toContain("memorySettings.sourceSection")
    expect(memorySource).not.toContain("记忆存储位置")
    expect(memorySource).not.toContain("Agent 使用情况")
    expect(memorySource).not.toContain("外部资料同步")
    expect(memorySource).not.toContain("Source connector 只负责")
    expect(memorySource).not.toContain("高级配置：记忆 Provider 配置")
    expect(agentConfigSource).not.toContain("请到“记忆”页修复 Provider adapter")
    expect(memorySource).not.toContain("setMemoryToolsRecall")
    expect(memorySource).not.toContain("setMemoryToolsList")
    expect(memorySource).not.toContain("<span>recall</span>")
    expect(memorySource).not.toContain("<span>list</span>")
    expect(memorySource).not.toContain("memory" + "Backend")
    expect(memorySource).not.toContain("memory" + "StorePath")
    expect(memorySource).not.toContain("memory" + ".backend")
    expect(memorySource).not.toContain("store" + "_path")
    expect(memorySource).not.toContain('<option value="' + 'sqlite">')
    expect(memorySource).not.toContain('<option value="' + 'postgres">')
    expect(memorySource).not.toContain('<option value="' + 'memory">')
  })
  ...
  it("keeps global model capability table maintenance on the other page", () => {
    const providersSource = readFileSync(join(tabsDir, "ProvidersTab.tsx"), "utf8")
    const otherSource = readFileSync(join(tabsDir, "OtherTab.tsx"), "utf8")
    const operationsSource = readFileSync(operationsPath, "utf8")

    expect(providersSource).not.toContain("model-capability-sync")
    expect(providersSource).not.toContain("saveCapabilitySyncSettings")
    expect(providersSource).not.toContain("refreshModelCapabilities")
    expect(otherSource).toContain("model-capability-sync")
    expect(otherSource).toContain("模型能力表维护")
    expect(otherSource).toContain("saveCapabilitySyncSettings")
    expect(otherSource).toContain("refreshModelCapabilities")
    expect(operationsSource).toContain('providers: ["providers", "modelProfiles"]')
    expect(operationsSource).toContain('other: ["serverSettings", "modelCapabilities"]')
  })
  ...
  it("keeps capabilities layout on shared settings primitives", () => {
    ...
    expect(capabilitiesSource).not.toContain(["capabilityPackage", "draft", "accept"].join("."))
    expect(capabilitiesSource).not.toContain("待确认草案")
  ...
  it("keeps adjacent settings tabs on shared page shell primitives", () => {
    ...
    expect(memorySource).toContain("SettingsPage")
    expect(memorySource).toContain("SettingsPageHeader")
    expect(memorySource).toContain("SettingsActionRail")
    expect(memorySource).toContain("SettingsFlatSection")
    expect(memorySource).toContain("SettingsSectionHeading")
    expect(otherSource).toContain("SettingsPage")
    expect(otherSource).toContain("SettingsPageHeader")
    expect(otherSource).toContain("SettingsActionRail")
    expect(otherSource).toContain("SettingsFlatSection")
    ...
    expect(memorySource).not.toContain('class="settings-page')
    expect(memorySource).not.toContain('class="settings-page-header')
    expect(memorySource).not.toContain("settings-actions settings-actions--right")
    expect(otherSource).not.toContain('class="settings-page')
    expect(otherSource).not.toContain('class="settings-page-header')
    expect(otherSource).not.toContain("settings-actions settings-actions--right")
  ...
  it("keeps capability packager model binding configurable through Agent settings", () => {
    const agentConfigSource = readFileSync(join(tabsDir, "AgentConfigTab.tsx"), "utf8")
    const controllerSource = readFileSync(join(__dirname, "useSettingsController.tsx"), "utf8")
    ...
    expect(agentConfigSource).not.toContain("SystemAgentConfig")
    expect(controllerSource).toContain("function modelParametersFromProfile")
    expect(controllerSource).toContain("modelPayload.parameters = parameters")
    expect(controllerSource).toContain("function systemAgentDraftToPayload")
    expect(controllerSource).toContain('draft.visibility === "user"')
  })
})
