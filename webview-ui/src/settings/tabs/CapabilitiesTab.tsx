...
export const CapabilitiesTab: Component<TabProps> = (props) => {
  ...
  const renderCapabilityDetail = (capability: CapabilityView) => (
    <>
      ...
      <Show when={capability.kind === "mcp_server" && capability.mcp}>
        ...
        >
          <div class="capability-detail-actions">
  ...
  return (
      <div class="settings-page settings-page--wide capability-dashboard-page">
        ...
        <section class="capability-workbench" classList={{ "settings-section--hidden": section() !== "capabilities" }}>
          <div class="capability-list-pane">
            <div class="capability-toolbar">
              <div class="capability-kind-tabs" role="tablist" aria-label="能力类型筛选">
                <For each={[
                  ["all", "全部"],
              ...
              <div class="capability-toolbar__search">
                <span class="codicon codicon-search" aria-hidden="true" />
                <input value={capabilitySearch()} placeholder="搜索 MCP Server、Skill、能力包或依赖" onInput={(event) => setCapabilitySearch(event.currentTarget.value)} />
              </div>
              <div class="capability-toolbar__actions" aria-label="能力设置">
          ...
          <aside class="capability-detail-pane">
        ...
        <div class="capability-summary-grid" classList={{ "settings-section--hidden": section() !== "dependencies" }}>
          <button class="capability-summary-card" classList={{ "is-active": capabilityStatusFilter() === "ready" }} onClick={() => setCapabilityStatusFilter(capabilityStatusFilter() === "ready" ? "all" : "ready")}>
            <span>已就绪</span>
            <strong>{String(dependencySummary().ready)}</strong>
          </button>
          <button class="capability-summary-card" classList={{ "is-active": capabilityStatusFilter() === "missing" }} onClick={() => setCapabilityStatusFilter(capabilityStatusFilter() === "missing" ? "all" : "missing")}>
            <span>未安装</span>
            <strong>{String(dependencySummary().missing)}</strong>
          </button>
          <button class="capability-summary-card" classList={{ "is-active": capabilityStatusFilter() === "stopped" }} onClick={() => setCapabilityStatusFilter(capabilityStatusFilter() === "stopped" ? "all" : "stopped")}>
            <span>未运行</span>
            <strong>{String(dependencySummary().stopped)}</strong>
          </button>
          <button class="capability-summary-card" classList={{ "is-active": capabilityStatusFilter() === "awaiting" }} onClick={() => setCapabilityStatusFilter(capabilityStatusFilter() === "awaiting" ? "all" : "awaiting")}>
            <span>待授权/待确认</span>
            <strong>{String(dependencySummary().awaiting)}</strong>
          </button>
        </div>

        <section class="capability-workbench" classList={{ "settings-section--hidden": section() !== "dependencies" }}>
          <div class="capability-list-pane">
            <p class="settings-empty-note">
              这里管理能力依赖：CLI、SDK、Runtime、凭据、路径等由能力引用的外部资源。MCP Server 和 Skill 在“能力”页管理。
            </p>
            <div class="capability-toolbar">
              <div class="capability-kind-tabs" role="tablist" aria-label="工具类型筛选">
                <button classList={{ "is-active": true }}>能力依赖</button>
              </div>
              <div class="capability-toolbar__search">
                <span class="codicon codicon-search" aria-hidden="true" />
                <input value={capabilitySearch()} placeholder="搜索工具、文档、命令" onInput={(event) => setCapabilitySearch(event.currentTarget.value)} />
              </div>
              <label class="field-label field-label--compact">
                <span>环境 Agent</span>
                <input value={environmentAgentLabel()} disabled />
              </label>
              <div class="capability-toolbar__actions" aria-label="新增能力依赖">
            ...
            <div class="capability-table" role="table" aria-label="能力依赖清单">
              <div class="capability-table__row capability-table__row--head" role="row">
                <span>资源名称</span>
                <span>{t("capability.filterKind")}</span>
                <span>来源/文档</span>
                <span>部署属性</span>
                <span>安装/运行状态</span>
                <span>操作</span>
              ...
              <Show when={filteredDependencyItems().length} fallback={<div class="capability-empty">没有匹配的能力依赖。</div>}>
                <For each={filteredDependencyItems()}>
                  {(item) => {
                    ...
                    return (
                      <div
                        class="capability-table__row capability-table__row--item"
                        classList={{ "is-selected": selectedDependencyId() === item.id }}
                        role="row"
                        tabIndex={0}
                        ...
                        onKeyDown={(event) => {
                          if (event.key === "Enter" || event.key === " ") {
                            ...
                            setSelectedDependencyId(item.id)
                          }
                        }}
                      >
                        <span class="capability-name-cell">
                          <strong>{item.name}</strong>
                          <small>{item.alias || item.command || "未记录别名"}</small>
                        </span>
                        <span><StatusBadge>{resourceKindLabel(displayResourceKind(item))}</StatusBadge></span>
                        <span class="capability-source-cell">{capabilitySourceLabel(item)}</span>
                        ...
                        <span class="capability-row-actions">
                          ...
                          >
                            <button class="ez-icon-button" title="编辑" onClick={(event) => { event.stopPropagation(); openEditCapability(record()) }}>
                              <span class="codicon codicon-edit" aria-hidden="true" />
                            </button>
                            <button class="ez-icon-button" title={item.enabled ? "停用" : "启用"} onClick={(event) => { event.stopPropagation(); enableCapability(record(), !item.enabled) }}>
                              <span class={`codicon codicon-${item.enabled ? "debug-pause" : "debug-start"}`} aria-hidden="true" />
                            </button>
                            <button class="ez-icon-button" title="删除" onClick={(event) => { event.stopPropagation(); deleteCapability(record()) }}>
              ...
              {(item) => (
                <>
                  ...
                  <div class="capability-detail-actions">
                    ...
                    >
                      <button class="btn btn-secondary" onClick={() => openEditCapability(dashboardItemToRecord(item()))}>
                        编辑
                      </button>
                    </Show>
                  </div>
