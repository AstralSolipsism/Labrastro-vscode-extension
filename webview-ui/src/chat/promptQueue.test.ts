...
describe("prompt queue", () => {
  it("sends the next queued prompt after a successful round", () => {
    const queued = enqueuePrompt(
      enqueuePrompt(createPromptQueueState(), "first", { id: "p1", createdAt: 1 }),
      "second",
      { id: "p2", createdAt: 2 }
    )
  ...
  it("pauses queued prompts after an error or cancellation", () => {
    const queued = enqueuePrompt(createPromptQueueState(), "follow up", { id: "p1", createdAt: 1 })
  ...
  it("resumes and clears queued prompts explicitly", () => {
    const paused = resolvePromptQueueAfterChat(
      enqueuePrompt(createPromptQueueState(), "follow up", { id: "p1", createdAt: 1 }),
      "error"
    ).state
